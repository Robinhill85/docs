...
By default, a [subagent](/oss/deepagents/subagents) sees only the task description you give it—it starts with no memory of the conversation that led up to the delegation. Set `mode: "fork"` on a subagent spec and it instead inherits the parent's full effective conversation history and exact system prompt, continuing as if it had been part of the conversation all along.
...
Add `mode: "fork"` to a subagent spec to match the @[`ForkedSubAgent`] spec. All other [`SubAgent`](/oss/deepagents/subagents#subagent-dictionary-based) fields apply—`name`, `description`, `tools`, `model`, `middleware`, `interrupt_on`, `permissions`, `response_format`—except two, which are rejected outright rather than silently ignored:
